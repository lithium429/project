...
<gd:PopLayout>
	...
	<div class="data_model data_cont_wrap pop_wrap">
		<form id="data_form" method="post" action="admin/user/update_psw.do" data-ajax="true" data-ajax-begin="showTip" data-ajax-complete="hideTip" data-ajax-success="backToList" data-ajax-success="showError" novalidate="novalidate">
			<input type="hidden" name="id" value="${model.id}" />
			<input type="hidden" name="password_hid" value="${model.password}" />
			<input type="hidden" name="salt" value="${model.salt}" />
			<table class="view wc100">
				<tbody>
					<tr>
						<th class="w18per"></span>用户名：</th>
						<td class="">
							${model.name}
						</td>
					</tr>
					<tr>
						<th><span class="c_red">*</span>原密码：</th>
						<td>
							<input class="inp_t" data-val="true"  data-val-required="请输入原密码！" id="password_old" name="password_old" type="password" />                        
			                      <span class="field-validation-valid" data-valmsg-for="password_old" data-valmsg-replace="true"></span>
