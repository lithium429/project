...
<gd:PopLayout>
	...
	<jsp:body>
		...
		<div class="data_model data_cont_wrap pop_wrap">
			<form id="data_form" method="post" action="admin/menu/add.do" data-ajax="true" data-ajax-begin="showTip" data-ajax-complete="hideTip" data-ajax-success="backToList" data-ajax-error="showError">
				<input type="hidden" name="layer" value="1" id="layer">
					...
					<tbody>
							...
							<td>
								<select id="parent_id" name="parent_id">
									...
									<c:forEach var="item" items="${menuList}" varStatus="vs">
										<option layer="${item.layer }" child_ids="${item.child_ids }"  value="${item.id}" <c:if test="${item.id eq model.parent_id}">selected="selected"</c:if> >
											${gdf:paddingSpace(item.layer)}${item.name}
										</option>
									</c:forEach>
								</select>
							</td>
						...
						<tr id="icon_tr">
							...
							<td id="icon_td">
								...
								<i id="icon_show" class="mail"></i>
							</td>
						</tr>
						<tr>
							<th>是否启用</th>
							<td>
								<label><input type="radio" value="true" class="is_enable" checked="checked" name="is_enable"  />是</label>
								<label><input type="radio" value="false" class="is_enable ml80" name="is_enable"  />否</label>
							</td>
						</tr>
					</tbody>
					<tfoot>
